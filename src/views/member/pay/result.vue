  ...
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <div class="pay-result">
        <span v-if="$router.query.payResult" class="iconfont icon-queren2 green"></span>
        <span v-else class="iconfont icon-shanchu red" ></span>
        <p class="tit">订单支付{{$router.query.payResult ? '成功': '失败'}}</p>
        <p class="tip">我们将尽快为您发货，收货期间请保持手机畅通</p>
        <p>支付方式：<span>支付宝支付</span></p>
        <p>支付金额：<span class="red" v-if="order">¥{{order.payMoney}}</span></p>
        <div class="btn">
          <!-- /member/order是查看订单列表 -->
          <XtxButton
            type="primary"
