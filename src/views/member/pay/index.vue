  ...
  <div class="xtx-pay-page">
    <div class="container">
      ...
      <div class="pay-info" v-if="order">
        ...
        <div class="tip">
          <p>订单提交成功！请尽快完成支付。</p>
          <p v-if="time > 0 && order.countdown > -1">
            支付还剩 <span>{{timeText}}</span>, 超时后将取消订单
          </p>
          <p v-else>订单已经超时，请重新下订单</p>
        </div>
        <div class="amount">
          <span>应付总额：</span>
...
export default {
  ...
  setup () {
    ...
    findOrderDetail(route.query.orderId).then(res => {
      ...
      // 订单剩余多长时间(res.result.countdown)，单位是秒
      // 当订单过期的时候，res.result.countdown值为-1
    ...
    // 使用时间倒计时的hooks函数
    const { time, timeText, start } = usePayTime()

    // 支付地址
    // const payUrl = '后台服务基准地址 + 支付页面地址 + 订单ID + 回跳地址'
    // 将回跳地址转为URI编码的格式 (因为这个地址有特殊字符串的拼接)
