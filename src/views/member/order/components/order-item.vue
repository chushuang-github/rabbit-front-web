<template>
  <div class="order-item">
    <div class="head">
      ...
      <!-- 已完成 已取消，显示删除 -->
      <a v-if="[5, 6].includes(order.orderState)" class="del" @click="$emit('on-delete', order)">删除</a>
    </div>
    <div class="body">
      ...
      <div class="column action">
        <!-- orderState：1为待付款；2为待发货；3为待收货；4为待评价；5为已完成；6为已取消 -->
        <!-- 1待付款(提交订单，但没有付款)：立即付款，查看详情，取消订单 -->
        <!-- 2待发货：查看详情，再次购买 -->
        <!-- 3待收货：确认收货，查看详情，再次购买 -->
        <!-- 4待评价：查看详情，再次购买，申请售后 -->
        <!-- 5已完成：查看详情，再次购买，申请售后 -->
        ...
        <XtxButton
          ...
          size="small"
        >确认收货</XtxButton>
        <p @click="$router.push(`/member/order/${order.id}`)">
