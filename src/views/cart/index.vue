      ...
      <div class="cart">
        <table>
          <thead>
            <tr>
              <th width="120">
                <XtxCheckbox :modelValue="$store.getters['cart/isCheckAll']" @change="checkAll">
                  全选
                </XtxCheckbox>
              </th>
          ...
          <tbody>
            ...
            <tr v-for="goods in $store.getters['cart/validList']" :key="goods.skuId">
              <td>
                <!-- 默认参数通过$event进行传递，下面两种方式都是ok的 -->
                <!-- <XtxCheckbox :modelValue="goods.selected" @change="checkOne(goods.skuId, $event)" /> -->
                <XtxCheckbox :modelValue="goods.selected" @change="selected => checkOne(goods.skuId, selected)" />
              </td>
              <td>
                <div class="goods">
              ...
              <td class="tc">
                <XtxNumbox :modelValue="goods.count" :max="goods.stock" @change="updateCount(goods.skuId, $event)" />
              </td>
              <td class="tc">
              ...
              <td class="tc">
                ...
                <p><a class="green" href="javascript:;" @click="delectCart(goods.skuId)">删除</a></p>
...
export default {
  ...
  setup () {
    ...
    // sku发生变化 (修改规格)
    // 需要将商品信息，新的skuId和老的skuId都传递过去
    // 用两个skuId进行判断，是新增的购物车商品，还是购物车本来就有的商品
